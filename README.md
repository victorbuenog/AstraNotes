...
| **Markdown** | Each note uses a **versioned document** with a **block** array; the MVP edits the primary **markdown** block. **Write / Split / Read** modes all keep markdown editable; preview uses the same in-memory note as you type (keystrokes update state before the debounced save). **Split** view has a **draggable** divider on wide layouts. **Enter** in a list continues list markers; a second **Enter** on an empty list line exits the list. GFM + inline/block LaTeX math via `react-markdown` + `remark-gfm` + `remark-math` + `rehype-katex` (`$...$`, `$$...$$`). Remote images in preview follow normal `<img>` rules (use HTTPS sources you trust). |
...
| **Theme / editor appearance** | **Light** / **dark** from **Settings**; styling uses CSS variables (`ThemeContext`). Settings also control note font family and note font size. |
| **Private vault PIN** | Set a PIN (4–12 digits) in **Settings**. Accessing the private vault always prompts for the PIN. Resetting the PIN wipes all existing private notes before the new PIN is applied. |
