...
| **FR3** | Users can **switch** between raw Markdown editing and a **rendered preview** (or split view) without losing unsaved work; preview behavior relative to “last saved vs editor buffer” is **documented**. | Maps to US-2; image/link policy must be explicit (security vs UX). |
